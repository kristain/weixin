...
<h4 class="left">新增场景 </h4>
	</div>
	<div class="tab">
		<ul>
			<li class="tabli" id="tab2"><a href="<?php echo U('Scenes/add');?>">新增场景</a>
			</li>
			<li class="current tabli" id="tab20"><a href="<?php echo U('Scenes/index');?>">场景管理</a>
			</li>
		</ul>
	</div>
	<div>
		<ul class="unstyled album_list">
			<?php if(is_array($set)): $i = 0; $__LIST__ = $set;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li>
					<a href="<?php echo U('Scenes/addtp',array('id'=>$item['id'],'token'=>$token));?>" class="album_pic">
						<img src="<?php echo ($item["catp"]); ?>">
					</a>
					<div class="album_main">
						<p class="album_title"><?php echo ($item["name"]); ?></p>
						<p class="album_title">浏览量：<?php echo ($item['click']/3); ?></p>
						<p>	<span class="pull-right">
				<a class="btnGray vm" href="<?php echo U('Scenes/addtp',array('id'=>$item['id'],'token'=>$token));?>" title="内页">内页</a>
				<a class="btnGray vm" rel="tooltip" href="<?php echo U('Scenes/edit',array('id'=>$item['id'],'token'=>$token));?>" title="编辑">编辑</a>
				
				<a class="btnGray vm" rel="tooltip" href="#" onclick="drop_confirm('您确定要删除吗?删除不可恢复', '<?php echo U('Scenes/del',array('id'=>$item['id'],'token'=>$token));?>');" title="删除">删除</a>
				<a class="btnGray vm" target="_blank" href="<?php echo ($f_siteUrl); ?>/index.php?g=Wap&m=Scenes&a=index&token=<?php echo ($token); ?>&id=<?php echo ($item["id"]); ?>" >预览</a>
				</span>
